---
title: "Web Search"
description: "Search the web in real-time with native internet access across all models"
---

import { APP_URL } from '/snippets/urls.mdx';

## Overview

All AI models in Cattus have native access to the internet, enabling them to search the web in real-time during your conversations. This capability ensures you receive up-to-date information, current events, and the latest data without leaving the chat interface.

<Card
  title="Try Web Search Now"
  icon="globe"
  href={APP_URL}
  horizontal
>
  Ask questions and get real-time answers from the web. Start free today.
</Card>

## How It Works

### Automatic Web Search

<video
  autoPlay
  muted
  loop
  playsInline
  className="w-full aspect-video rounded-xl"
  src="/videos/web-search.mp4"
>
</video>

<Steps>
  <Step title="Ask Your Question">
    Simply ask questions that require current information or real-time data.
  </Step>

  <Step title="Model Searches Automatically">
    The AI model detects when web search is needed and automatically searches the internet for relevant information.
  </Step>

  <Step title="Get Current Answers">
    Receive responses based on the latest information available online, with sources when applicable.
  </Step>
</Steps>

### Native Integration

Web search is built directly into all models, meaning:

- **No Additional Steps**: You don't need to enable or configure anything
- **Seamless Experience**: The model decides when to search based on your query
- **Universal Access**: Available across all AI models in Cattus
- **Always Current**: Get the most recent information available online

## Use Cases

<Accordion title="Market Research">
  Ask about current market trends, competitor analysis, or industry news. The model will search for and synthesize the latest information from multiple sources.
</Accordion>

<Accordion title="Technical Documentation">
  Get the most recent documentation, API changes, or software updates for your development needs.
</Accordion>

<Accordion title="Academic Research">
  Find recent publications, studies, or academic papers relevant to your research topic.
</Accordion>

<Accordion title="Travel Planning">
  Check current travel advisories, weather conditions, local events, or updated travel restrictions.
</Accordion>

## Best Practices

### When Web Search Is Used

Models automatically use web search for queries that benefit from real-time information:

<CardGroup cols={2}>
  <Card title="Current Events" icon="newspaper">
    Get the latest news, updates, and breaking stories from around the world.
  </Card>
  <Card title="Real-Time Data" icon="chart-line">
    Access live stock prices, weather forecasts, sports scores, and other time-sensitive data.
  </Card>
  <Card title="Recent Information" icon="clock">
    Find information about recent developments, new releases, or updated guidelines.
  </Card>
  <Card title="Fact Checking" icon="magnifying-glass">
    Verify current facts, statistics, or claims with the most recent sources available.
  </Card>
</CardGroup>

### Crafting Effective Queries

To get the best results from web search:

1. **Be Specific**: Include relevant details like dates, locations, or specific topics
2. **Indicate Recency**: Use phrases like "latest," "current," or "recent" when appropriate
3. **Ask Directly**: Frame questions naturally as you would when searching online
4. **Request Sources**: Ask the model to provide sources when you need to verify information

### Examples

<CodeGroup>

```text Good Query
What are the latest features announced for Python 3.13?
```

```text Good Query
What's the current price of Tesla stock?
```

```text Good Query
What are the most recent climate change studies from 2025?
```

</CodeGroup>

## Tips & Tricks

<Tip>
All models in Cattus have this capability by default. You don't need to switch to a specific model or enable any settings to use web search.
</Tip>

<Info>
The model will determine when web search is necessary based on your query. For queries that don't require current information, it may rely on its training data instead.
</Info>

<Warning>
While web search provides access to current information, always verify critical information from authoritative sources, especially for important decisions.
</Warning>

## Start Using Web Search

<CardGroup cols={2}>
  <Card
    title="Get Started Free"
    icon="rocket"
    href={APP_URL}
  >
    Launch Cattus and start asking questions with real-time web search.
  </Card>
  <Card
    title="Explore All Features"
    icon="grid"
    href="/features/models"
  >
    Discover how web search works with multi-model selection and other features.
  </Card>
</CardGroup>
